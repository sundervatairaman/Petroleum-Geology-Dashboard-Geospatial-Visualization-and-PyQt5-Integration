...
#import geologplots_ploty_qt_sub1 as gp
#from geologplots_ploty_qt_sub1 import allplot as ap
#from geologplots_ploty_qt_sub1 import Plot_sw2 as Plot_sw2
import panel as pn
...
class FloatSlider(QSlider):
    """Custom QSlider that handles float values."""
    def __init__(self, start, end, step, value, format='%.2f', *args, **kwargs):
        super().__init__(Qt.Horizontal, *args, **kwargs)
        self.setMinimum(0)
        self.setMaximum(int((end - start) / step))
        self.step = step
        self.start = start
        self.format = format
        self.setValue(int((value - start) / step))

    def value_float(self):
        return self.start + self.value() * self.step

class SliderPanel(QWidget):
    def __init__(self):
        super().__init__()

        # Create a vertical layout for the slider panel
        layout = QVBoxLayout()

        # Sliders for different parameters
        self.dtclean_slider = FloatSlider(40, 140, 1.0, 90, format='%.2f')
        self.dtshale_slider = FloatSlider(40, 140, 1.0, 110, format='%.2f')
        self.spclean_slider = FloatSlider(-100, 25, 0.5, -50, format='%.2f')
        self.spshale_slider = FloatSlider(-100, 25, 0.5, 0, format='%.2f')
        self.grclean_slider = FloatSlider(0, 150, 0.5, 45, format='%.2f')
        self.grshale_slider = FloatSlider(0, 150, 0.5, 120, format='%.2f')
        self.neutshale_slider = FloatSlider(0, 0.60, 0.01, 0.32, format='%.2f')

        # Labels to display values of sliders
        self.dtclean_label = QLabel(f"DT Clean: {self.dtclean_slider.value_float():.2f}")
        self.dtshale_label = QLabel(f"DT Shale: {self.dtshale_slider.value_float():.2f}")
        self.spclean_label = QLabel(f"SP Clean: {self.spclean_slider.value_float():.2f}")
        self.spshale_label = QLabel(f"SP Shale: {self.spshale_slider.value_float():.2f}")
        self.grclean_label = QLabel(f"GR Clean: {self.grclean_slider.value_float():.2f}")
        self.grshale_label = QLabel(f"GR Shale: {self.grshale_slider.value_float():.2f}")
        self.neutshale_label = QLabel(f"Neutron Shale: {self.neutshale_slider.value_float():.2f}")

        # Add sliders and labels to the layout
        layout.addWidget(self.dtclean_label)
        layout.addWidget(self.dtclean_slider)
        layout.addWidget(self.dtshale_label)
        layout.addWidget(self.dtshale_slider)
        layout.addWidget(self.spclean_label)
        layout.addWidget(self.spclean_slider)
        layout.addWidget(self.spshale_label)
        layout.addWidget(self.spshale_slider)
        layout.addWidget(self.grclean_label)
        layout.addWidget(self.grclean_slider)
        layout.addWidget(self.grshale_label)
        layout.addWidget(self.grshale_slider)
        layout.addWidget(self.neutshale_label)
        layout.addWidget(self.neutshale_slider)
        ...
        # Initialize a flag to track if slider values have changed
        self.values_changed = False

        # Connect sliders to update labels in real-time and set the flag for recalculation
        self.dtclean_slider.valueChanged.connect(self.handle_slider_change)
        self.dtshale_slider.valueChanged.connect(self.handle_slider_change)
        self.spclean_slider.valueChanged.connect(self.handle_slider_change)
        self.spshale_slider.valueChanged.connect(self.handle_slider_change)
        self.grclean_slider.valueChanged.connect(self.handle_slider_change)
        self.grshale_slider.valueChanged.connect(self.handle_slider_change)
        self.neutshale_slider.valueChanged.connect(self.handle_slider_change)

        # Connect the "Recalculate" button to trigger recalculation
        self.recalculate_button.clicked.connect(self.handle_recalculate_button)

    # Update label functions
    def update_dtclean_label(self):
        self.dtclean_label.setText(f"DT Clean: {self.dtclean_slider.value_float():.2f}")

    def update_dtshale_label(self):
        self.dtshale_label.setText(f"DT Shale: {self.dtshale_slider.value_float():.2f}")

    def update_spclean_label(self):
        self.spclean_label.setText(f"SP Clean: {self.spclean_slider.value_float():.2f}")

    def update_spshale_label(self):
        self.spshale_label.setText(f"SP Shale: {self.spshale_slider.value_float():.2f}")

    def update_grclean_label(self):
        self.grclean_label.setText(f"GR Clean: {self.grclean_slider.value_float():.2f}")

    def update_grshale_label(self):
        self.grshale_label.setText(f"GR Shale: {self.grshale_slider.value_float():.2f}")

    def update_neutshale_label(self):
        self.neutshale_label.setText(f"Neutron Shale: {self.neutshale_slider.value_float():.2f}")

    # Handle slider change to update labels and set the flag for recalculation
    def handle_slider_change(self):
        # Update all labels
        self.update_dtclean_label()
        self.update_dtshale_label()
        self.update_spclean_label()
        self.update_spshale_label()
        self.update_grclean_label()
        self.update_grshale_label()
        self.update_neutshale_label()

        # Set the flag to indicate that values have changed
        self.values_changed = True

    # Handle the "Recalculate" button click event
    def handle_recalculate_button(self):
        # Only recalculate if values have changed
        if self.values_changed:
            self.recalculate()
            self.values_changed = False  # Reset the flag after recalculation
        else:
            print("No values changed. Recalculation not needed.")

    # Example recalculation function
    def recalculate(self):
        # Get the slider values
        dt_clean = self.dtclean_slider.value_float()
        dt_shale = self.dtshale_slider.value_float()
        sp_clean = self.spclean_slider.value_float()
        sp_shale = self.spshale_slider.value_float()
        gr_clean = self.grclean_slider.value_float()
        gr_shale = self.grshale_slider.value_float()
        neut_shale = self.neutshale_slider.value_float()

        # Example of recalculating based on the values
        print(f"Recalculating with values - DT Clean: {dt_clean}, DT Shale: {dt_shale}, SP Clean: {sp_clean}, SP Shale: {sp_shale}, GR Clean: {gr_clean}, GR Shale: {gr_shale}, Neutron Shale: {neut_shale}")

        # Here you would add the calls to your plotting or calculation functions
        # For example:
        #shale_plot3(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_clean, dt_shale)

        # Replace 'shale_parameters.txt' with the desired file path
        file_path = './data/parameters/shale_parameters.txt'
        self.write_shale_parameters_to_file(file_path, sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_clean, dt_shale)
   

        # Display or return the recalculated values (for now we print them)
        return dt_clean, dt_shale, sp_clean, sp_shale, gr_clean, gr_shale, neut_shale

    # Method to return the current slider values
    def get_values(self):
        if self.values_changed:
            self.recalculate()

        return (
            self.dtclean_slider.value_float(),
            self.dtshale_slider.value_float(),
            self.spclean_slider.value_float(),
            self.spshale_slider.value_float(),
            self.grclean_slider.value_float(),
            self.grshale_slider.value_float(),
            self.neutshale_slider.value_float()
        )

    def write_shale_parameters_to_file(self,file_path, sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_clean, dt_shale):
      try:
...
class Slider_Panel1(QWidget):
    def __init__(self):
        ...
        self.m_slider = FloatSlider(1.00, 3.00, 0.01, 1.9)
        self.n_slider = FloatSlider(1.00, 3.00, 0.01, 2.0)
        self.rw_slider = FloatSlider(0.01, 0.1, 0.001, 0.15, format='%.3f')
        self.mslope_slider = FloatSlider(0.01, 4.00, 0.01, 1.5, format='%.3f')

        # Labels to display the values
        self.m_label = QLabel(f"Cementation Exponent 'm_cem': {self.m_slider.value_float():.2f}")
        self.n_label = QLabel(f"Saturation Exponent 'n_sat': {self.n_slider.value_float():.2f}")
        self.rw_label = QLabel(f"Rw: {self.rw_slider.value_float():.3f}")
        self.mslope_label = QLabel(f"m* Slope: {self.mslope_slider.value_float():.3f}")
        ...
        layout.addWidget(self.rw_label)
        layout.addWidget(self.rw_slider)
        layout.addWidget(self.mslope_label)
        layout.addWidget(self.mslope_slider)
        ...
        # Connect signals to update labels on value change and set values_changed flag
        self.m_slider.valueChanged.connect(self.handle_slider_change)
        self.n_slider.valueChanged.connect(self.handle_slider_change)
        self.rw_slider.valueChanged.connect(self.handle_slider_change)
        self.mslope_slider.valueChanged.connect(self.handle_slider_change)

        # Connect the "Recalculate" button to the recalculation method
        self.recalculate_button.clicked.connect(self.handle_recalculate_button)

        # Initialize a flag to track if any slider's value has changed
        self.values_changed = False

    # Update label functions
    def update_m_label(self):
        self.m_label.setText(f"Cementation Exponent 'm_cem': {self.m_slider.value_float():.2f}")

    def update_n_label(self):
        self.n_label.setText(f"Saturation Exponent 'n_sat': {self.n_slider.value_float():.2f}")

    def update_rw_label(self):
        self.rw_label.setText(f"Rw: {self.rw_slider.value_float():.3f}")

    def update_mslope_label(self):
        self.mslope_label.setText(f"m* Slope: {self.mslope_slider.value_float():.3f}")
    ...
    def recalculate(self):
        # Get the slider values
        m_value = self.m_slider.value_float()
        n_value = self.n_slider.value_float()
        rw_value = self.rw_slider.value_float()
        mslope_value = self.mslope_slider.value_float()

        # Example of recalculating based on the values
        print(f"Recalculating with values - m: {m_value}, n: {n_value}, Rw: {rw_value}, m* Slope: {mslope_value}")

        # Here you would add the calls to your plotting or calculation functions
        # For example:
        #ap =  allplot( min_depth,max_depth)

        #ap.Plot_sw2(m_value, n_value, rw_value, mslope_value)
        # Plot_sw2( m, n, rw, mslope )

        # Replace 'Pickett.txt' with the desired file path
        file_path = './data/parameters/Pickett.txt'
        self.write_constants_to_file(file_path, m_value, n_value, rw_value, mslope_value)



        # Display or return the recalculated values (for now we print them)
        return m_value, n_value, rw_value, mslope_value
    ...
    def get_values(self):
        ...
        return (
            self.m_slider.value_float(),
            self.n_slider.value_float(),
            self.rw_slider.value_float(),
            self.mslope_slider.value_float(),
        )

    def write_constants_to_file(self,file_path, m_cem, n_sat, Rw, mslope):
      try:
          with open(file_path, 'w') as file:
              file.write("m_cem: {}\n".format(m_cem))
              file.write("n_sat: {}\n".format(n_sat))
...
class RangeSlider(QWidget):
    def __init__(self):
        ...
        self.min_slider = QSlider(Qt.Vertical)  # Vertical slider for minimum value
        self.max_slider = QSlider(Qt.Vertical)  # Vertical slider for maximum value

        # Set slider ranges
        self.min_slider.setMinimum(1200)
        self.min_slider.setMaximum(1779)
        self.min_slider.setValue(1300)

        self.max_slider.setMinimum(1200)
        self.max_slider.setMaximum(1779)
        self.max_slider.setValue(1650)

        # Invert the minimum slider's appearance (top to bottom)
        self.min_slider.setInvertedAppearance(True)
        self.max_slider.setInvertedAppearance(True)

        # Create labels to display the selected range
        self.min_label = QLabel(f"Min: {self.min_slider.value()}")
        self.max_label = QLabel(f"Max: {self.max_slider.value()}")

        # Create the "Recalculate" button
        self.recalculate_button = QPushButton("Recalculate")

        # Layout for the sliders and labels (vertical for each slider + label)
        vbox_min = QVBoxLayout()
        vbox_max = QVBoxLayout()

        # Add the min slider and its label to the layout
        vbox_min.addWidget(self.min_label)
        vbox_min.addWidget(self.min_slider)

        # Add the max slider and its label to the layout
        vbox_max.addWidget(self.max_label)
        vbox_max.addWidget(self.max_slider)

        # Horizontal layout to place sliders side by side
        hbox_sliders = QHBoxLayout()
        hbox_sliders.addLayout(vbox_min)
        hbox_sliders.addLayout(vbox_max)

        # Vertical layout to include sliders and button
        vbox_main = QVBoxLayout()
        vbox_main.addLayout(hbox_sliders)
        vbox_main.addWidget(self.recalculate_button)

        # Set the main layout for the widget
        self.setLayout(vbox_main)

        # Connect slider events to update the labels and set flag when sliders change
        self.min_slider.valueChanged.connect(self.handle_slider_change)
        self.max_slider.valueChanged.connect(self.handle_slider_change)

        # Connect the "Recalculate" button to trigger recalculation
        self.recalculate_button.clicked.connect(self.handle_recalculate_button)

        # Initialize a flag to track if either slider's value has changed
        self.values_changed = False

    def update_min_slider(self):
        # Ensure the min slider does not exceed the current max slider value
        if self.min_slider.value() > self.max_slider.value():
            self.min_slider.setValue(self.max_slider.value())
        self.min_label.setText(f"Min: {self.min_slider.value()}")

    def update_max_slider(self):
        # Ensure the max slider does not go below the current min slider value
        if self.max_slider.value() < self.min_slider.value():
            self.max_slider.setValue(self.min_slider.value())
        self.max_label.setText(f"Max: {self.max_slider.value()}")

    def handle_slider_change(self):
        # Update the sliders and enforce constraints
        self.update_min_slider()
        self.update_max_slider()
    ...
    def recalculate(self,data ):
        min_depth = self.min_slider.value()
        max_depth = self.max_slider.value()
        ...
        ###
        #gp.allplot(min_depth, max_depth)
        ap =  allplot( min_depth,max_depth)


        
        update_plot(min_depth, max_depth)
        #triple_combo_plot(min_depth, max_depth)
        #logs = data[(data.DEPT >= min_depth) & (data.DEPT <= max_depth)]

        #logs1 = logs[(logs["DEPT"] >= min_depth) & (logs["DEPT"] <= max_depth)]
        #logs=logs1

        #knn_Porosity_Ploty2(min_depth, max_depth)
        #Ploty_lith5(min_depth, max_depth)
        #shale_plot3(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale)
        #Plot_sw2( m, n, rw, mslope )
        #Wax_Smits_Plot_plotly(min_depth,max_depth)
        #final_plot(top_summary, bottom_summary)
        #Plot_lith2(min_depth, max_depth)

        print(f"Recalculating... Min Depth: {min_depth}, Max Depth: {max_depth}")
        return self.min_slider.value(), self.max_slider.value()


    # Method to return min and max depths
    def get_depths(self):
...
def update_plot(min_depth, max_depth):
    # Function to handle plotting or calculations with the depth range
    print(f"Updating plot with min depth: {min_depth} and max depth: {max_depth}")
    # You can add code to update your plot here based on the depth range.
    # For example, filtering data or updating the plot's x-axis range.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.corl_html_file2 = os.path.abspath("http://127.0.0.1:8050/")
        ...
        # Create a QSplitter to divide the window into left (sliders) and right (web content)
        splitter = QSplitter()

        # Create the left panel (contains slider panels)
        left_panel = QWidget()
        left_layout = QVBoxLayout()  # Vertical layout to stack slider panels

        # Instantiate slider panels and range slider (placeholders)
        self.slider_panel = SliderPanel()        # First set of sliders
        self.slider_panel1 = Slider_Panel1()     # Second set of sliders
        self.range_slider = RangeSlider()        # Range slider (min-max)

        # Apply background colors to each panel
        self.slider_panel.setStyleSheet("background-color: #f0f0f0;")  # Light gray for SliderPanel
        self.slider_panel1.setStyleSheet("background-color: #e0f7fa;")  # Light cyan for Slider_Panel1
        self.range_slider.setStyleSheet("background-color: #fff3e0;")   # Light orange for RangeSlider

        # Add the slider panels to the left panel layout
        left_layout.addWidget(self.slider_panel)
        left_layout.addWidget(self.slider_panel1)
        left_layout.addWidget(self.range_slider)

        # Button to trigger rerunning the logic (added to left_layout)
        self.button = QPushButton("Rerun Logic")
        left_layout.addWidget(self.button)  # Add the button to the left layout

        # Set the layout for the left panel
        left_panel.setLayout(left_layout)

        # Create the right panel (a QWebEngineView for Plotly graph or web content)
        right_panel = QWidget()
        ...
        # Add both left panel (sliders) and right panel (webview + dropdown) to the splitter
        splitter.addWidget(left_panel)
        splitter.addWidget(right_panel)

        # Set initial sizes for the splitter panels (left, right)
        splitter.setSizes([50, 1400])  # 300px for left (sliders), 900px for right (webview)

        # Set the splitter as the central widget of the main window
        self.setCentralWidget(splitter)

        # Connect the button to rerun logic
        self.button.clicked.connect(self.rerun_logic)

        # Initial run of the logic
        self.rerun_logic()

        # Launch Dash app in a separate thread
        #self.launch_dash_app()


    def shale_plot3A(self,sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale):
    # Define the layout with sliders on the left and plot on the right
      ...
      app.layout = html.Div([
        html.Div([
            html.Label('SP Clean'),
            dcc.Slider(id='sp-clean-slider', min=0, max=150, step=1, value=sp_clean,
                       marks={i: str(i) for i in range(0, 151, 50)}),

            html.Label('SP Shale'),
            dcc.Slider(id='sp-shale-slider', min=0, max=150, step=1, value=sp_shale,
                       marks={i: str(i) for i in range(0, 151, 50)}),

            html.Label('DT Clean'),
            dcc.Slider(id='dt-clean-slider', min=0, max=150, step=1, value=dt_matrix,
                       marks={i: str(i) for i in range(0, 151, 50)}),

            html.Label('DT Shale'),
            dcc.Slider(id='dt-shale-slider', min=0, max=150, step=1, value=dt_shale,
                       marks={i: str(i) for i in range(0, 151, 50)}),

            html.Label('GR Clean'),
            dcc.Slider(id='gr-clean-slider', min=0, max=150, step=1, value=gr_clean,
                       marks={i: str(i) for i in range(0, 151, 50)}),

            html.Label('GR Shale'),
            dcc.Slider(id='gr-shale-slider', min=0, max=150, step=1, value=gr_shale,
                       marks={i: str(i) for i in range(0, 151, 50)}),

            html.Label('Neutron Shale'),
            dcc.Slider(id='neutron-shale-slider', min=0, max=150, step=1, value=neut_shale,
                       marks={i: str(i) for i in range(0, 151, 50)}),
        ], style={'width': '30%', 'display': 'inline-block', 'vertical-align': 'top', 'padding': '20px'}),

        # Plotly graph on the right side
        html.Div([
            dcc.Graph(id='shale-plot')
      ...
      # Callback function to update the plot based on the slider values
      @app.callback(
      ...
      def update_plot(sp_clean, sp_shale, dt_clean, dt_shale, gr_clean, gr_shale, neut_shale):
        fig = shale_plot3(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_clean, dt_shale)
        return fig
    ...
    def change_html_file(self):
        """Update the QWebEngineView to display the selected HTML file or Dash URL."""
        selected_html = self.dropdown.currentData()
        if selected_html.startswith("http"):  # If it's a URL (Dash app)
            self.webview.setUrl(QUrl(selected_html))
        else:  # Otherwise, treat it as a local file
            self.webview.setUrl(QUrl.fromLocalFile(selected_html))

    def launch_dash_app(self):
        """Launch Dash app in a new thread."""
        global dash_thread
        dash_thread = Thread(target=self.run_dash_app)
        dash_thread.start()

    def run_dash_app(self):
        """Run the Dash app."""
        self.shale_plot3A(sp_clean=0, sp_shale=0, gr_clean=0, gr_shale=0, neut_shale=0, dt_matrix=0, dt_shale=0)

    def rerun_logic(self):
        print("Rerunning logic...")
...
min_depth, max_depth = range_slider.get_depths()



amn = Slider_Panel1()
m_value, n_value, rw_value, mslope_value = amn.get_values()



Shale = SliderPanel()
dt_clean, dt_shale, sp_clean, sp_shale, gr_clean, gr_shale, neut_shale = Shale.get_values()
...
def allplot( min_depth,max_depth):
  ...
  Shale = SliderPanel()
  dt_clean, dt_shale, sp_clean, sp_shale, gr_clean, gr_shale, neut_shale = Shale.get_values()
  print(dt_clean, dt_shale, sp_clean, sp_shale, gr_clean, gr_shale, neut_shale)
  ...
  amn = Slider_Panel1()
  m_value, n_value, rw_value, mslope_value = amn.get_values()
  print(m_value, n_value, rw_value, mslope_value )
...
# Function to define layout and bind sliders to update the plot
  def shale_plot3A(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale):
    # Define the layout with sliders on the left and plot on the right
    app.layout = html.Div([
        html.Div([
            html.Label('SP Clean'),
  ...
  # Function to start Dash in a separate thread
  def run_dash_app():
  ...
  webview.setUrl(QUrl("http://127.0.0.1:8050/"))
  ...
  # Function to define layout and bind sliders to update the plot
  def shale_plot3AA(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale):
    # Define the layout with sliders on the left and plot on the right
    app.layout = html.Div([
        html.Div([
            html.Label('SP Clean'),
    ...
    def update_plot(sp_clean, sp_shale, dt_clean, dt_shale, gr_clean, gr_shale, neut_shale):
        # Call shale_plot3 function with updated slider values
        fig = shale_plot3(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_clean, dt_shale)
        return fig
  ...
  ###########################################################
  def shale_plot3A1(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale):
    # Define the sliders for the shale parameters
    sp_clean_slider = pn.widgets.FloatSlider(name='SP Clean', value=sp_clean, start=0, end=150)
    sp_shale_slider = pn.widgets.FloatSlider(name='SP Shale', value=sp_shale, start=0, end=150)
    dt_clean_slider = pn.widgets.FloatSlider(name='DT Clean', value=dt_matrix, start=0, end=150)
    dt_shale_slider = pn.widgets.FloatSlider(name='DT Shale', value=dt_shale, start=0, end=150)
    gr_clean_slider = pn.widgets.FloatSlider(name='GR Clean', value=gr_clean, start=0, end=150)
    gr_shale_slider = pn.widgets.FloatSlider(name='GR Shale', value=gr_shale, start=0, end=150)
    neut_shale_slider = pn.widgets.FloatSlider(name='Neutron Shale', value=neut_shale, start=0, end=150)

    # Create a Panel layout for sliders
    sliders = pn.Column(
        sp_clean_slider, sp_shale_slider,
        dt_clean_slider, dt_shale_slider,
        gr_clean_slider, gr_shale_slider,
        neut_shale_slider
    )

    # Create an update function to pass dynamic slider values to shale_plot3
    def update_plot(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale):
        fig = shale_plot3(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale)
        return pn.pane.Plotly(fig, height=900)

    # Use pn.bind to link the update function with slider values
    reactive_plot = pn.bind(
        update_plot, sp_clean=sp_clean_slider, sp_shale=sp_shale_slider, 
        gr_clean=gr_clean_slider, gr_shale=gr_shale_slider, 
        neut_shale=neut_shale_slider, dt_matrix=dt_clean_slider, dt_shale=dt_shale_slider
    )

    # Arrange the layout: sliders on the left and plot on the right
    layout = pn.Row(sliders, reactive_plot)

    # Display the layout
    #layout.show()
    pn.serve(layout, port=63677, show=True)  # This will open http://localhost:63677/

    # Optionally save the layout to an HTML file
    #layout.save('shale_plot_interactive.html', embed=True)  # Use embed=True to include JavaScript and CSS
    print("Layout saved as shale_plot_interactive.html")
  ...
  def shale_plot3B(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_matrix, dt_shale):
    ...
    # Create a Panel to display both sliders and the Plotly plot together
    panel_plot = pn.pane.Plotly(fig,height=900)

    # Link the sliders to the update function using pn.bind
    update_bound = pn.bind(shale_plot3, sp_clean=sp_clean_slider, sp_shale=sp_shale_slider, 
    ...
    # Arrange the layout
    layout = pn.Row(
        pn.Column(*sliders),  # Use unpacking to add all sliders
        panel_plot  # Display plot next to sliders
    )
  ...
  shale_plot3A(sp_clean, sp_shale, gr_clean, gr_shale, neut_shale, dt_clean, dt_shale)
  ...
  Plot_sw2(m_cem, n_sat, Rw, mslope)
